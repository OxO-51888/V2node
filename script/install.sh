...
install_v2node() {
    ...
    if  [[ -z "$version_param" ]] ; then
        last_version=$(curl -Ls "https://api.github.com/repos/OxO-51888/V2node/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        url="https://github.com/OxO-51888/V2node/releases/download/${last_version}/v2node-linux-${arch}.zip"
