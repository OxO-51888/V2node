...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/OxO-51888/V2node/main/script/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/OxO-51888/V2node/main/script/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/v2node -N --no-check-certificate https://raw.githubusercontent.com/OxO-51888/V2node/main/script/v2node.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/OxO-51888/V2node ---
  ${green}0.${plain} 修改配置
————————————————
